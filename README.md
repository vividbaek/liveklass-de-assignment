...
6. SQL 집계 쿼리 작성 완료
7. 다음 단계: matplotlib 기반 PNG 시각화 생성
...
사용자, 강의, 결제 테이블을 별도로 정규화하지 않고 `event_type`과 `session_id`를 기준으로 모든 행동 이벤트를 하나의 테이블에 저장했습니다.
이후 SQL 분석 단계에서 session funnel, 강의별 매출, 오류 비율 같은 분석용 결과를 가공합니다.
...
분석 목적에 따라 funnel, revenue, retention mart 테이블로 가공하는 방식이 일반적입니다.
본 과제에서는 데이터 규모와 재현성을 고려해 단일 `events` 테이블을 선택했습니다.
...
현재 포함된 분석 쿼리는 다음과 같습니다.

1. 이벤트 타입별 발생 수
2. 강의별 매출
3. `checkout_started` → `purchase_completed` 전환율
4. 무료 체험 300초 이상 시청 session의 구매 전환율
5. 오류 영역별 발생 수
