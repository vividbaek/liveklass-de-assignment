...
따라서 Kubernetes를 바로 구현하기보다는, 운영 환경에서 어떤 부분에 적용할 수 있는지 판단하는 수준으로 정리했습니다.
...
이번 과제에서는 Kubernetes manifest를 바로 추가하지 않는 것이 더 낫다고 판단했습니다.
이유는 다음과 같습니다.

- 현재 app은 실제 ingestion API server가 아니라 synthetic event generator 중심입니다.
- Kubernetes YAML을 추가해도 실제 cluster에서 검증하지 않으면 형식적인 산출물이 될 수 있습니다.
...
실제로 manifest를 만든다면 처음부터 많은 YAML을 추가하기보다 최소 단위만 작성하는 것이 좋습니다.
...
이 파일들은 실제 cluster 배포용이라기보다,
현재 Docker container가 Kubernetes에서 어떤 실행 단위로 바뀌는지 보여주는 예시로 시작하는 것이 안전합니다.
